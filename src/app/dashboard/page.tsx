...
//       `${item.produto} (Qtd: ${item.quantidade}, R$ ${item.preco.toFixed(2)})`
//     ).join('; ');
...
//       'Produtos': produtos,
//       'Quantidade Total': sale.itens.reduce((sum, item) => sum + item.quantidade, 0)
//     };
//   });
...
const Dashboard: React.FC = () => {
  ...
  const fetchSalesData = async (): Promise<Sale[]> => {
    try {
      ...
      const formattedSales: Sale[] = data.map((sale: any) => ({
        id: sale.id,
        data: sale.createdAt, // Supondo que o Prisma retorna createdAt
  ...
  useEffect(() => {
    ...
    // Verificar mudanças periodicamente (opcional, dependendo do caso de uso)
    // const interval = setInterval(processData, 30000); // Aumentado para 30 segundos para evitar muitas requisições
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2 ව2">Dashboard de Vendas</h1>
            <p className="text-gray-600">Dados do sistema PDV em tempo real</p>
          </div>
          <div className="flex gap-3">
            <button
